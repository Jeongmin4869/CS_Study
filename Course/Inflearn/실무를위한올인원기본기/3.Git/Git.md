...
## 3. 전략적 Git 사용 - Gitflow
   
### 01 - Gitflow 브랜치 전략
   
1. 필요한 기능을 분석, 정의하고 개발한다.
2. 개발이 완료되면 테스트 환경에 배포한다.
3. 통합 테스트를 진행하여 버그 등 문제가 없는지 확인한다.
4. 문제가 있다면 이를 수정한 후 2~3의 과정을 거친다.
5. 테스트 완료 후 최종적으로 운영 환경에 배포한다.
6. 최종 배포 후 심각한 버그를 발견할 경우 수정하여 업데이트된 프로젝트에 배포한다. 
   
<img width="1400" height="860" alt="Image" src="https://github.com/user-attachments/assets/d07d3cef-0b1c-42cf-922f-fe869d9e838b" />
   
#### Master
- 운영 환경에 최종 배포가 되는 프로젝트를 담는 브랜치

#### Develop
- 기능 개발을 완료한 프로젝트를 담는 브랜치
- Master 브랜치로부터 파생되며 master 브랜치와 항상 싱크를 맞춘다 
   
#### Feature
- 필요한 각 기능을 작업하는 브랜치
- develop브랜치로부터 파생
- 해당 브랜치에서 작업을 완료하면 develop 브랜치로 머지
   
#### Release
- 최종 배포 전 통합 테스트를 할 프로젝트를 담는 브랜치
- Develop 브랜치로부터 파생
- 테스트중 발견한 버그 수정
- 테스트가 완료되면 master브랜치와 develop 브런치에 머지
   
### 브랜치 흐름
1. Master 브랜치에서 develop브랜치를 만든다.
2. 필요한 기능 사항들을 정의하고 개발자들 별로 개발 담당을 정한다.
3. 기능 사항 별로 각 개발자는 develop브랜치에서 feature 브랜치를 만들어 작업한다.
4. Feature 브랜치에서 작업 완료 후 remote repository에 push하여 팀원에게 리뷰를 받는다
5. 리뷰가 완료된 feature 브랜치는 develop 브랜치로 머지한다.
6. 정의했던 기능 사항을 모두 개발하면 develop브런치에서 release브랜치를 만든다.
7. Release 블랜치 위에서 통합 테스트를 진행한다.
8. 통합 테스트를 마치면 mater브랜치와 develop브랜치를 머지한다.
9. Master 브랜치의 최종 커밋에 tag를 달아 프로젝트 버전을 명시한 후 배포한다.
10. 최종 배포된 프로젝트에 심각한 버그가 발견되었을 때 master 브랜치에서 hotfix 브랜치를 만들어 버그 수정 작업을 진행한다.
11. 버그 수정 완료 후 master 브랜치와 develop 브랜치에 머지한다.
12. 버그 수정이 완료된 master브랜치의 브로젝트의 최종 커밋에 tag를 달아 프로젝트 버전을 명시한다. 
...
#### 단점
- 여러단계의 머지작업을 거치기 때문에 머지 커밋이 늘어난다. 
